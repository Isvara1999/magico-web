...
import { CaretDownIcon, MountainsIcon, CheckCircleIcon, AirplaneTiltIcon, MapPinIcon, StarIcon, SunIcon } from '@phosphor-icons/react';
import { useLanguage } from '../../contexts/LanguageContext';

const FAQ_ICONS = [MountainsIcon, CheckCircleIcon, AirplaneTiltIcon, MapPinIcon, StarIcon, SunIcon];

const VueloDelCondorFAQ: React.FC = () => {
  const { t } = useLanguage();
    ...
    <section className="py-16 md:py-24 px-6 bg-[#F9F8F4]">
      <div className="max-w-4xl mx-auto">
        <div className="mb-12" data-reveal>
          <p className="text-brand font-bold tracking-widest uppercase text-xs mb-3 opacity-80">Antes de volar</p>
          <h2 className="text-3xl md:text-5xl serif-title brand-green mb-3">{t.vuelo_condor.faq.title}</h2>
          <p className="text-gray-500 text-base md:text-lg font-light">{t.vuelo_condor.faq.subtitle}</p>
        </div>

        <div className="space-y-3 md:space-y-4">
          {t.vuelo_condor.faq.items.map((faq: any, index: number) => {
...
export default VueloDelCondorFAQ;
